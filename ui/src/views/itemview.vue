...
export default {
  ...
  setup() {
    ...
    function deleteItem(item) {
      confirm.require({
        header: 'Verwijderen van artikelen',
        message: `Ben je zeker dat je "${item.description}"(${item.labelid}) in de vuilnisbak wil zetten?`,
        icon: 'mdi mdi-alert-outline',
        accept: () => {
          items.remove({ ...item, deleted: true });
        },
        reject: () => {
          info({
            summary: 'Verwijderen van een artikel',
            detail: `Het artikel "${item.description}"(${item.labelid}) is niet verwijderd.`,
          });
        },
      });
    }
